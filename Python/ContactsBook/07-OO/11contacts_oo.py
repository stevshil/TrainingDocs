...
def add_user():
    ...
    local_contact= contact.contact(contact_name,contact_phone,contact_dob)
...
def display_contacts(local_contacts,message):
    contact_list={}
    counter=0
    for contact in local_contacts:  # Enumerate returns the index and the value of the list
        print(f"{counter}: {contact.get_name()}")
        contact_list[counter]=contact.get_name()
        counter+=1
    ...
    try:
        ...
        my_contact=local_contacts[get_option]
        print(f"Name: {my_contact.get_name()}")
        print(f"Mobile: {my_contact.get_phone()}")
        print(f"D.O.B.: {contact.get_dob()}")
    except Exception:
        # Loop to menu
        return False
...
while chose != 4:
    ...
    if chose == 1:
        ...
        if tmp_contacts == False:
            ...
            my_contacts.set_contact(tmp_contacts)

    elif chose == 2:

        get_name = display_contacts(my_contacts.get_contacts(),"delete")
        if get_name == False:
            continue
        ...
        if ( response == "y" ):
            my_contacts.delete_contact(get_name)
    
    elif chose == 3:
        get_name = display_contacts(my_contacts.get_contacts(),"display")
